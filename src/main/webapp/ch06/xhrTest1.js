var xhrObject; //XMLHttpRequest객체를 지정할 변수를 전역변수로 선언
...
function createXHR() { //XMLHttpRequest객체를 생성하는 메소드
	if (window.ActiveXObject) {
		...
		//웹 브라우저가 IE7.0이상, 크롬, 파이어폭스, 사파리, 오페라등의 경우 XMLHttpRequest 객체 생성
...
function resultProcess(){//응답결과가 오면 자동으로 실행
	    ...
	    //서버로 부터 응답받는 HTTP상태가 정상인 경우 수행
        //문자열로 이루어진 서버의 응답을 받아서, id의 값이 "dispalyArea"에 표시
